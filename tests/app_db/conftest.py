...
directory's fixtures (test_build_app_db.py, test_deals.py - KAN-6/KAN-7) are
tiny synthetic databases with no loose produce in them at all, and
build_produce raises loudly if the checked-in, real 750-row/242-row files
reference a generic key a build's own data never produced - which a tiny
fixture never will.

Point PRODUCE_UNITS_TSV/PRODUCE_GENERIC_MAP_TSV at empty (header-only) files
for every test under this directory, so KAN-6/KAN-7's fixtures keep testing
